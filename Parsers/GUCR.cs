...
namespace errchk.Parsers {
    ...
    public class GUCR : Plugin {

        public override string Key => "GUCR";
        public override string Name => "GUCR Parser";
        ...
        private (int Errors, int Warnings) PrintReport(string fullpath, List<Models.Row> rows, bool warnings) {
            ...
            ColorConsole.WriteWarning("LINE   EC_USERNAME                      DETAILS");
            ColorConsole.WriteWarning("-----------------------------------------------------------------");
            foreach (var row in rows)
            {
                if (row.details.Contains("already created")){
                    _warnings++;
                    if (warnings) {
                        ColorConsole.WriteWarning($"{row.Line.ToString("D4")} - {row.ec_username.PadRight(30, ' ')} - {row.details}");
                    }
                }
                else {
                    _errors++;
                    ColorConsole.WriteError($"{row.Line.ToString("D4")} - {row.ec_username.PadRight(30, ' ')} - {row.details}");
                }
            }
